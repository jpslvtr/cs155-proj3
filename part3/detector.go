		...
		for packet := range packetSource.Packets() {
			...
			if tcpLayer != nil && ipLayer != nil && etherLayer != nil {
				...
				*/
				...
				src := strconv.Itoa(int(arp.SourceHwAddress[0])) + "." + strconv.Itoa(int(arp.SourceHwAddress[1])) + "." + strconv.Itoa(int(arp.SourceHwAddress[2])) + "." + strconv.Itoa(int(arp.SourceHwAddress[3]))
				dst := etherLayer.(*layers.Ethernet).DstMAC.String()
